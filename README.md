...
| **瀑布流作品集** | CSS multi-column 瀑布流，78 张作品横竖混排自适应，比例钳制在 `[0.8, 1.6]` |
...
| **灯箱 / 英雄轮播回归** | `NODE_PATH=<workspace>/node_modules node _tools/_check_lightbox.js`（jsdom DOM 级断言，覆盖「切换不抽一下」「静态基准 = 动画终态」「EXIF 抽字段」等 33 项） |
...
- **新增**｜人像作品第二批次：18 张社员原创实拍（`portrait-04` ~ `portrait-21`，含国风、自然光、舞台、校园写真等），站点作品总数 60 → **78 张**、人像 3 → **21 张**。
- **新增**｜首页「人像与视觉」独立章节（Chapter Ⅳ）：文案 + 古宅横构图主图 + 6 张精选瀑布流，与「查看全部人像作品」入口联动；导航与统计数字同步更新为 78 张 / 4 大题材。
- **新增**｜构建脚本自动抽取拍摄信息（EXIF）：`_tools/build_assets.py` 现从原图抽取 `机型 / 镜头 / 光圈 / 快门 / ISO / 焦段 / 日期`，写入每条作品的 `shot` 字段；78 张中 61 张已带拍摄信息，灯箱底部自动展示（缺字段自动隐藏该行）。
- **新增**｜上传素材统一入口：构建脚本新增 `u()` 助手，指向 `摄影社素材（网站）/上传网站/`，新批次只改清单不碰旧路径。
- **修复**｜玉武三江首页两条展示图均替换为指定照片（风雨桥上的迎宾队伍）：全出血大图 `hero-field` 与 Chapter Ⅱ 主图 `sanjiang-01`，文案同步改为「风雨桥上的迎宾队伍」。
- **修复**｜首页 Hero 轮播切换「二次抽动」：旧实现每次切图都 `animation:none → void offsetWidth → animation:""` 强制重挂 Ken Burns，画面先缩一下再推近。改为静态基准态 `scale(1.01)` 与 Ken Burns 终态**严格对齐** + 交叉淡入（下层带 `transition-delay`），移除 JS 对动画的强制重挂；并预解码下一张，杜绝「半张图刷出来」的假抽动。（`home.js` / `main.css`）
- **修复**｜灯箱切换「二次抽动」：同样病根（重挂 `lbIn` 动画）。改为只做极短透明度过渡（`is-step` 类，不碰 `transform`），静态基准态与 `lbIn` 终态一致；切换完成后摘除 `is-step`，恢复入场动画。（`site.js` / `main.css`）
- **新增**｜`_tools/_check_lightbox.js`：灯箱 / 轮播回归断言（jsdom），33 项全通过，覆盖「切换不碰 transform」「静态基准 = 动画终态」「EXIF 字段格式」等关键契约，配合 `_check_since.js` 形成前端回归套件。
